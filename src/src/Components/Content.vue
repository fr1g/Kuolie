    ...
    <div class=" w-full m-auto p-1.5 md:p-5 grid justify-items-center transition-all" ref="rend" id="ROUND">
        <div :class="`  bg-${this._()}-200 text-${this._(1)}-800 relative
            ...
            <h1 class="px-1.5 flex w-full flex-wrap" style="height: 36.5px;">
                <div class="grow">
                    <input id="INPUT" v-model="titlePass" ref="titl" style="height: 36.5px;" type="text"
                        :class="'  w-full inline-block overflow-x-visible text-3xl border-0 outline-1 bg-transparent transition '"
                        placeholder="点击输入标题文本..." />
                    <span id="OUTPUT" style="height: 36.5px;" class="text-3xl hidden!   w-full transition"
                        v-html="(this.titlePass.length > 0 ? this.titlePass : '无标题之Kuolie.kami.su~').replaceAll(this.reg, '').replaceAll(this.regFrame, '')"></span>
                </div>
                <div class=" w-fit block -top-1 -right-1 select-none text-sm scale-50 md:scale-90 "
                    :style="this.Config.showInfo.stat ? `opacity: .3;` : `display: none !important; --tw-scale-x: .65; --tw-scale-y: .65; `">
                    {{ this.watermark }}
                </div>
            </h1>
            <div id="bs" ref="texts" :style="`--id-show: ${this.Config.squareId.stat ? 'block' : 'hidden'};`" :class="` gap-3.5 transition-all 
                    ...
                    `">
                <TextBox v-for="s of this.TextBoxes" :x="s.x" :key="GetPosition(s)" :id="GetPosition(s)"
                    :displayId="(ignorePID ? s.id : GetPosition(s))" :offset="Config.waterfall.stat ? s.offset : -1"
                    :span="s.span" :inside="s.content" :fill="Config.waterfall.stat ? false : Config.fill.stat"
                    :isPlaceHolder="s.isPlaceHolder" :self="s" :showingBlockId="Config.squareId.stat"
                    :isMasonry="Config.waterfall.stat" :extendInfo="s.extendInfo" />
                <!-- need minimize -->
            </div>
        </div>
    </div>
