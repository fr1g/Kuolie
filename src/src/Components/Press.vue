<template>
    <div :title="((this.title ?? this.link))" :class="(`pressable transition ${this.unspaced <= 1 || (this.overclass ?? 'x').includes('text-') ? '' : 'text-sm'} ${((this.overclass ?? '').includes('bg-')) ? '' : 'bg-stone-300 dark:bg-stone-700'}
          text-stone-950  dark:text-stone-50 shadow${(this.initShadow ?? 'true') == 'true' ? '' : (this.initShadow == 'sm' ? '-sm' : '-none')} 
            ...
            @todo:bg-opacity bg--/${((this.initOpacity != null) & (typeof parseInt(this.initOpacity) == 'number')) ? this.initOpacity : '35'} 
            @todo:dark-bg-opacity dark:bg--/${((this.initOpacity != null) & (typeof parseInt(this.initOpacity) == 'number')) ? this.initOpacity : '35'}
            hover:shadow-md @todo:hover-bg-opacity hover:bg--/70   active:shadow-sm @todo:active-bg-opacity active:bg--/100 
            dark:@todo:hover-bg-opacity hover:bg--/70 ?dark:hover:shadow-md dark:shadow ?dark:active:shadow-sm dark:@todo:active-bg-opacity active:bg--/100
        ...
        <span :class="`tense  ${(this.isForModalClose ?? 'false') == 'true' ? 'xxx' : 'modal-insider'} `">
            <i
                :class="(`${this.iconClass} ${(this.isForModalClose ?? 'false') == 'true' ? 'xxx' : 'modal-insider'} `)"></i>
            <span
                :class="` ${(this.isForModalClose ?? 'false') == 'true' ? 'xxx' : 'modal-insider'} text-center my-auto mx-0 ml-0 ${(this.isOnlyIcon ?? 'false') == 'true' ? ' use-icon ' : ' '}`"><span
                    ref="iconRef"
                    :class="` ${(this.isForModalClose ?? 'false') == 'true' ? 'xxx' : 'modal-insider'} use-icon translate-y-0.5 mr-1 ${this.iconRefHide ? 'hidden' : 'inline-block'}`"></span><span
                    :class="`inline-block ${(this.isForModalClose ?? 'false') == 'true' ? 'xxx' : 'modal-insider'} `">
                    <slot></slot>
                </span>{{ (((this.hideProtocol ?? 'false') == 'true') & this.ex.includes('://') ?
                    this.ex.split('://')[1] : this.ex) }}</span><i
                :class="`${(((this.noLinkIcon ?? 'false') == 'true') ? 'hidden' : ((this.type == 'outer' || this.isVerified == 'true') ? '  text-sm use-icon sm-icon' : 'hidden'))}  ${(this.isForModalClose ?? 'false') == 'true' ? 'xxx' : 'modal-insider'} `"
                style="vertical-align: text-top !important; " ref="endIconRef"> &#xe8a7;</i>
        </span>
    </div>
</template>
<script>
