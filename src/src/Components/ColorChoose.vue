<template>
    <div class="grid grid-cols-1 gap-1.5 overflow-y-hidden overflow-x-visible h-full p-1.5?">
        <div class="grid grid-cols-1 gap-1.5 p-1.5">
            ...
            `">
                <Icon class="scale-95 translate-x-0.5!">e790</Icon>
            </div>
        </div>
        <!-- 要求能够在超级扩展模式下处理三个颜色: 保存在localStorage: 三个颜色代码, 如果包含-则视作原装的 -->
        <div class="grid grid-cols-1 gap-1.5 h-full overflow-y-auto overflow-x-hidden rounded-lg p-1.5 pt-1">
            <div v-for="color of this.colors" :key="color" :title="color.replaceAll(/-\d{3}/g, '')" @click="C(color)"
                :id="`COLORBTN:${color.toLowerCase()}`" :class="`w-6 h-6 z-20 transition hover:scale-110 scale-105 active:scale-100 box-border overflow-hidden
            ...
            bg-${processMixColor(color.toLowerCase(), 0)}-500 `">
                ...
                bg-${color.includes('&') ? `${processMixColor(color.toLowerCase(), 1)}  shadow-sm` : 'transparent'}`">
                </div>
            </div>
...
import ColorSuperModeModal from './Views/ColorSuperModeModal.vue';

export default {
    name: 'ColorChoose',
    components: { Icon, ColorSuperModeModal },
    mounted() {
    ...
    methods: {
        ...
        ConfSuperModeModal: function () {
            this.Modal(true, this.plusModal);
        }
