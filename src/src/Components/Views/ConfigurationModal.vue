<template>
    <div class="modal-view max-h-[90vh] h-full">
        ...
        <div class=" overflow-x-hidden overflow-y-auto ">
            ...
            <div class="toggle modal-view grid grid-cols-3 p-1.5 gap-1.5">
                <div :class="`modal-view border rounded-lg grid grid-cols-3 gap-1 p-1.5 @todo:bg-opacity bg--/40 @todo:dark-bg-opacity dark:bg--/40  bg-gray-50 dark:bg-gray-700 shadow hover:shadow-md transition-all `"
                    v-for="i of global" :key="i.name">
                    <span
                        class=" p-1.5 block w-fit font-semibold col-span-2 h-fit align-bottom m-auto modalx -translate-y-0.5">{{
                            i.name }}</span>
                    <div class="modalx text-right">
                        <Press v-if="i.type != 'input'"
                            :class="` ${i.confirmed == undefined ? '' : (i.confirmed ? 'bg-red-500' : 'bg-red-300')}    p-1.5 w-fit mt-0.5 scale-90 modal-view-x border-zinc-700 border-2 shadow-none hover:shadow-none active:shadow-none inline-block`"
                            :overclass="`shadow-none hover:shadow-none active:shadow-none   ${i.confirmed == undefined ? '' : (i.confirmed ? 'bg-red-500' : 'bg-red-300')}   `"
                            @click.native="Configure(i.key, !i.stat)"
                            :style="i.confirmed == undefined ? '--tw-bg-opacity: 0' : '--tw-bg-opacity: .7'"
                            init-opacity="70">
                            <Icon class="modal-view  translate-y-0.5">{{ i.stat == null ? (i.confirmed ? 'e814' :
                                'e7b5') : (i.stat ? 'f78c' : 'f78a') }}</Icon>
                        </Press>
                        <input v-if="i.key == 'groundOpacity'" type="number" v-model="ground" max="100" min="0"
                            @input="Configure('groundOpacity')" title="数字, 小于100大于0" placeholder="数字, 小于100大于0"
                            class="w-full px-1.5 translate-y-1 inline-block rounded-lg border-2 bg-slate-100 @todo:bg-opacity bg--/30" />
                    </div>
            ...
            <div class="modal-view font-semibold text-lg mt-1">
                渲染区设置
                <br>
                <span class="text-base font-normal @todo:bg-opacity bg--/70">
                    设置将会在页面刷新后恢复默认.
                </span>
            </div>
            <div class="toggle modal-view grid grid-cols-3 p-1.5 gap-1.5">
                <div :class="`modalxxx px-0.5 flex flex-wrap  modal-view border rounded-lg @todo:bg-opacity bg--/40 @todo:dark-bg-opacity dark:bg--/40  bg-gray-50 dark:bg-gray-700  shadow hover:shadow-md transition-all `"
                    v-for="i of conf" :key="i.name">
                    <span class="grow pl-1.5 block modalx translate-y-0.5">{{ i.name }}</span>
                    <Press class="w-fit modal-view-x  shadow-none hover:shadow-none active:shadow-none block"
                        overclass="shadow-none hover:shadow-none active:shadow-none"
                        @click.native="Config(i.key, !i.stat)" style="--tw-bg-opacity: 0">
