<template>
    <div id="main-area"
        :class="`min-h-screen bg-zinc-50 dark:bg-zinc-900 w-full relative transition  ${this.modal ? ' pointer-events-none' : ''}`"
        :style="`--opa: ${this.opa / 100}; `">
        <div class="grid sm:hidden justify-items-center justify-center items-center h-screen">
            <div class="text-lg px-8 text-zinc-900 dark:text-zinc-100">
                <p
                    :class="`text-3xl font-semibold text-${this._(1)}-800 dark:text-${this._(1)}-100 mb-3 pb-1 border-b w-5/6`">
                    窗口太小啦!</p>
                <p>用的是手机吗? 或许<b class=" font-semibold">横屏</b>查看就可以了哦! <br><span
                        class="text-base -translate-y-1.5 font-light opacity-90 inline-block italic">不然就用平板或者电脑吧...</span>
                </p>
                <p>用的就是电脑? 把浏览器窗口改为<b class=" font-semibold">最大化</b>吧!</p>
                <p>再不然我也没办法了xwx</p>
            </div>
            <div class="bottom-0 left-1 right-1 fixed mb-1">
                <p v-if="(!ua.includes('hrome') && ua.includes('afari'))"
                    class="text-center text-zinc-900 dark:text-zinc-100 text-sm scale-90 mb-0.5 opacity-75">
                    建议不要用Safari浏览器！实际上<br>iOS上所有浏览器实质上都是Safari</p>
                <p class="text-center opacity-70 animate-pulse text-zinc-900 dark:text-zinc-100"><b
                        class="font-bold">Kuolie</b> by Feiron Iguista <Icon class="text-rose-300 inline-block "
                        figure="0">eb52</Icon>
                </p>
            </div>
        </div>
        <div class="hidden sm:block">
            ...
            <div :class="`block ${this.groundOpacityPreference ?? 50}%  w-full min-h-screen m-auto relative transition pb-72 `"
                id="draw-area" :style="`
                    opacity: ${this.modal ? this.opa / 100 : '1'}; 
                    background-color: color-mix(in srgb, var(--color-${this._()}-50) ${this.groundOpacityPreference ?? 50}%, transparent) !important;
                `">
                <Content ref="c" />
            </div>
            <div id="palette" :style="`opacity: ${this.opa / 100};`"
                class="transition fixed right-1.5 top-1.5 bottom-1.5 left-auto p-1.5? shadow rounded-lg bg-neutral-100 dark:bg-neutral-700 h-full max-h-screen overflow-x-hidden overflow-y-auto">
                <ColorChoose />
            </div>
            <div :style="`opacity: ${this.opa / 100}; z-index: 998`"
                class="transition fixed w-full bg-zinc-200 dark:bg-zinc-800 bottom-0 left-0 right-0 min-h-fit shadow-lg drop-shadow-lg"
                id="input-area">
                <InputPanel ref="input" />
            </div>
            <div ref="modal" id="modal" :style="`opacity: ${(1 - (this.opa / 100)) * 2}; z-index: 999; `"
                :class="`${this.modal ? 'fixed' : 'hidden'} bg-${this._()}-100 top-10 override scale-90 bottom-10 left-1 right-1 m-auto p-5 rounded-lg shadow-lg transition h-fit min-h-fit?max-h-[60vh] w-2/3`">
                <div class="absolute top-1 right-0">
                    <Press is-for-modal-close="true" init-opacity="0" overclass="shadow-0" class="scale-75 inline-block"
                        style="box-shadow: none;" @click.native="_ => { _ }">
                        <Icon>e711</Icon>
                    </Press>
                </div>
                <ModalBase ref="modal-render" :class="`mt-3 text-${this._(1)}-800 ?dark:text-${this._(1)}-100 h-full`"
                    id="modal-render" :assembly="this.modalContent"
